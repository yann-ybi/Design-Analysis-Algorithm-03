...
# A 24-tuple of integers defines a Permutation
Permutation = Tuple[int, int, int, int, int, int, int, int, int, int, int, int,
                    int, int, int, int, int, int, int, int, int, int, int, int]

# Position and Permutation are represented with the same structure, in the world of code
# but we treat them differently in the world of ideas.
Position = Tuple[int, int, int, int, int, int, int, int, int, int, int, int,
                 int, int, int, int, int, int, int, int, int, int, int, int]
...
def perm_apply(permutation: Permutation, position: Position) -> Position:
    """
    Apply a Permutation to a Position and return the new Position.
    Face in position p[i] moves to position i.
    """
    # noinspection PyTypeChecker
    return tuple(position[i] for i in permutation)


def perm_inverse(permutation: Permutation) -> Permutation:
    """
    Given a Permutation, calculate and return the inverse of that Permutation.
    """
...
def perm_to_string(permutation: Permutation) -> str:
    """
    Given a Permutation, convert it to a string for compact printing.
    """
    return '(' + ' '.join((f'{p:2d}' for p in permutation)) + ')'
...
I: Position = (flu, luf, ufl, fur, urf, rfu, fdl, dlf, lfd, frd, rdf, dfr,
